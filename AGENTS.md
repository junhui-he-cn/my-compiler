...
- Supported expressions include literals, arrays, maps, ranges, structs, indexing, index-assignment validation, field access, field assignment, variables, calls, function expressions, exhaustive primitive and enum `match` expressions, grouping, unary operators, binary/logical operators, assignment expressions, and numeric compound assignment for variables, array elements, and struct fields.
- Enum variants may declare aligned named payloads such as `Ok(value: number)`; constructors remain positional, while variant patterns may use reordered `value: pattern` arguments. Match statements use block arms; match expressions use comma-separated `pattern [if condition] => expression` arms with an optional trailing comma. Both forms require exhaustive coverage from unguarded arms or a wildcard/binding arm, support `nil`, `true`, `false`, number, and string literal patterns, nullable primitive and enum patterns, nested positional and named variant patterns, and OR patterns using `|`. OR alternatives must bind the same names with mutually compatible types; successful alternatives share one arm-local binding. Boolean literal matches cover both `true` and `false`; open number and string domains require a wildcard or binding. Guards use existing truthiness and are checked in the arm-local binding scope without contributing coverage. Match expression arm results must have compatible types.
