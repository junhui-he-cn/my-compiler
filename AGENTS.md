...
- Top-level `import "path";` directives load dependency files relative to the importing file. Imported files have module-private top-level scope; standalone `export name[, name...];` lists expose selected already-defined top-level declarations to importers, while private declarations remain visible only inside the imported file. Duplicate canonical imports are no-ops, imports from stdin are rejected, and this phase has no namespace import syntax, re-exports, package search paths, or separate compilation.
