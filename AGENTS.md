...
- Supported statements include `let`, `print`, `if`/`else`, `while`, C-style `for`, `break`, `continue`, `fun`, `return`, top-level `import`, blocks, and expression statements.
...
- `break;` exits the nearest enclosing `while` or C-style `for`; `continue;` skips to the next condition check of the nearest enclosing `while` or runs the increment before the next condition check of the nearest enclosing `for`. Both are type errors outside loops, including inside nested function bodies that are lexically inside a loop.
