...
- Runtime values currently include nil, numbers, booleans, strings, functions, arrays, maps, and structs.
...
The active roadmap in `docs/roadmap.md` is now language-focused. The near-term sequence is runtime source locations and call stacks, Unicode string semantics, generic function/type abstraction, the completed first map collection slice, ranges, then algebraic data types and pattern matching. Broader iteration is not part of the active plan. Bytecode VM follow-ups such as GC, task scheduling, and JIT exploration are deferred backend tracks; start them only from a dedicated backend design spec and implementation plan. When adding language features, prefer vertical slices that update parser, AST, type checker, IR, bytecode, interpreters/VM, docs, and goldens together.
