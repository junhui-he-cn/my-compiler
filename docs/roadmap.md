...
Completed seventh slice: nullable annotations use postfix `?`, so `nil` is assignable to `T?` while non-nullable `T` remains protected. Nullable narrowing is implemented for direct `if` nil-checks and conservative logical `&&` / `||` combinations on one or more simple variables; broader flow-sensitive narrowing remains future work.
