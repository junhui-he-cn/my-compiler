# Agent Memory File Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a root-level `AGENTS.md` that gives future Codex/AI agents concise, project-specific development guidance for this compiler.

**Architecture:** Add a single repository-root memory file rather than changing compiler code or test tooling. The file will summarize module responsibilities, safe extension workflows, golden test conventions, verification commands, current semantics, and likely future work.

**Tech Stack:** Markdown documentation, C++17 compiler demo, CMake/CTest, Python 3 golden test runner.

---

## File Structure

- Create: `AGENTS.md` — project-level instructions for AI agents working in this repository.
- Verify: `docs/superpowers/specs/2026-07-01-agent-memory-design.md` — approved design source.
- Do not modify compiler source, build scripts, tests, README, or grammar in this change.

## Task 1: Create the Agent Memory File
...
```bash
sed -n '1,220p' docs/superpowers/specs/2026-07-01-agent-memory-design.md
```

Expected: output includes these lines:

```text
Create a project-level memory file for future Codex/AI-agent development.
Use a root-level `AGENTS.md` as the primary memory file.
```

- [ ] **Step 2: Create `AGENTS.md` with project-specific guidance**
...
Expected: `AGENTS.md` exists and contains project-specific agent guidance.
...
Expected: output starts with `# Agent Development Notes` and includes sections named `Architecture Map`, `Language Extension Workflow`, `Backend / IR Extension Workflow`, and `Golden Test Conventions`.

## Task 2: Verify and Commit the Documentation Change

**Files:**
- Verify: `AGENTS.md`
- Commit: `AGENTS.md`

- [ ] **Step 1: Scan for unfinished placeholder markers**
...
Expected: output includes `?? AGENTS.md`. It may also include `?? tests/__pycache__/` if Python tests have created bytecode; remove that before committing.

- [ ] **Step 3: Remove Python bytecode cache if present**
...
Expected: output includes `?? AGENTS.md` and does not include `tests/__pycache__/`.
...
```bash
git add AGENTS.md
git commit -m "docs: add agent development notes"
```
