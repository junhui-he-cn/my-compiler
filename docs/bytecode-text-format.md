...
`native_call` invokes a registered VM native stdlib function by name-table reference; in this version `push`, `pop`, `remove`, `keys`, `values`, `floor`, `ceil`, `sqrt`, `str`, `substr`, `charAt`, `typeOf`, `contains`, `slice`, `copy`, and `concat` are supported.
