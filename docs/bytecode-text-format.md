...
`native_call` invokes a registered VM native stdlib function by name-table reference; in this version `push`, `pop`, `remove`, `clear`, `keys`, `values`, `floor`, `ceil`, `sqrt`, `str`, `substr`, `charAt`, `typeOf`, `contains`, `slice`, `copy`, `concat`, `map`, `filter`, `any`, `all`, `count`, `find`, and `reduce` are supported.
